...
namespace Layton.AuditWizard.Common
{
	...
	public class UltraTree_DropHightLight_DrawFilter_Class : Infragistics.Win.IUIElementDrawFilter
	{
		...
		//The DropHighLightNode is a reference to the node the Mouse is currently over
		...
		public UltraTreeNode DropHightLightNode
		{
			...
			set 
			{
				//If the Node is being set to the same value, just exit
		...
		public DropLinePositionEnum DropLinePosition
		{
			...
			set
			{
				//If the position is the same as it was, just exit
		...
		//The BackColor of the DropHighLight node. This only affect the node when it is being dropped On. 
		//Not Above or Below. 
		...
		//The ForeColor of the DropHighLight node. This only affect the node when it is being dropped On. 
		//Not Above or Below. 
		...
		// Determines how close to the top or bottom edge of a node the mouse must be to be consider dropping 
		// Above or Below respectively. 
		// By default the top 1/3 of the node is Above, the bottom 1/3 is Below, and the middle is On. 
		...
		// When the DropNode or DropPosition change, we fire the Invalidate event to let the program know to invalidate
		// the Tree control. This is neccessary since the DrawFilter does not have a reference to the Tree Control 
		...
		//Set the DropNode to Nothing and the position to None. This will clear whatever Drophighlight is in the tree
		public void ClearDropHighlight()
		{
			SetDropHighlightNode(null, DropLinePositionEnum.None);
		}

		//Call this proc every time the DragOver event of the Tree fires. 
		//Note that the point pass in MUST be in Tree coords, not form coords
		public void SetDropHighlightNode(UltraTreeNode Node, System.Drawing.Point PointInTreeCoords )
		{
			//The distance from the edge of the node used to determine whether to drop Above, Below, or On a node
		...
		{
			//Use to store whether there have been any changes in DropNode or DropLinePosition
			bool IsPositionChanged = false;

			try	
			{
				//Check to see if the nodes are equal and if the dropline position are equal
			...
			{
				//If we reach this code, it means mvarDropHighLightNode is null, so it could not be compared
				if (mvarDropHighLightNode == null)
				{
					//Check to see if Node is nothing, so we'll know if Node = mvarDropHighLightNode
