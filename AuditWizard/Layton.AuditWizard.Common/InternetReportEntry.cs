...
{
	public class InternetReportEntry
	{
		#region Data
		private int			_InternetReportEntryID;
		...
		public int InternetReportEntryID
		{
			get { return _InternetReportEntryID; }
			set { _InternetReportEntryID = value; }
		}
		...
		public InternetReportEntry()
		{
			_InternetReportEntryID = 0;
			_location = "";
			_assetname = "";
			_source = "";
			_url = "";
		...
		public InternetReportEntry(DataRow dataRow)
		{
			try
			{
				this.InternetReportEntryID = (int)dataRow["_AUDITEDITEMID"];
				...
				if (categoryParts.Count < 4)
					return;
				_source = categoryParts[2];
			
				// Handle Internet History Records first
				if (_source == "History")
				{
					_date = Convert.ToDateTime(categoryParts[3]);
					_url = categoryParts[4];
			...
			catch (Exception ex)
			{
				...
				Utility.DisplayErrorMessage("Exception occurred creating an INTERNETREPORTENTRY Object, please check database schema.  The message was " + ex.Message);
			}
		}
