...
--																		--
-- This stored procedure is part of the AuditWizard Product				--
-- It is used to return statistics about asset support contract	 			--
--																		--
--	NOTE: THIS SP ONLY HANDLES ASSETS								--
--																		--
--------------------------------------------------------------------------
--																		--
-- History																--
-- =======																--
--																		--
--	28-March-2008	Sojan E John		Enhancement Support Contract for assets							--
--																		--
--------------------------------------------------------------------------

CREATE PROCEDURE [dbo].[usp_support_statistics_assets]
AS 
	DECLARE @dtToday DateTime
	DECLARE @dtThisWeek DateTime
	DECLARE @dtThisMonth DateTime
	...
	SELECT 'row'
	,(SELECT COUNT(_ASSETID) FROM ASSET_SUPPORTCONTRACT WHERE _ALERTFLAG=1 AND (DATEDIFF(day, @dtToday,_SUPPORT_EXPIRY) < 0)) expired

	,(SELECT COUNT(_ASSETID) FROM ASSET_SUPPORTCONTRACT WHERE _ALERTFLAG=1 AND (DATEDIFF(day, @dtToday,_SUPPORT_EXPIRY) = 0)) expiretoday

	,(SELECT COUNT(_ASSETID) FROM ASSET_SUPPORTCONTRACT WHERE _ALERTFLAG=1 AND (DATEDIFF(day,@dtToday,_SUPPORT_EXPIRY) > 0) AND (DATEDIFF(day, @dtThisWeek, _SUPPORT_EXPIRY) <= 0)) expirethisweek

	,(SELECT COUNT(_ASSETID) FROM ASSET_SUPPORTCONTRACT WHERE _ALERTFLAG=1 AND (DATEDIFF(day, @dtToday,_SUPPORT_EXPIRY) > 7) AND (DATEDIFF(day, @dtThisMonth, _SUPPORT_EXPIRY) <= 0)) expirethismonth
GO
...
CREATE PROCEDURE [dbo].[usp_supplierid_bysuppliername]
	@cName varchar(510),
	@nReturnID int output
AS
	--SET nocount ON
	SET @nReturnID = isnull((SELECT _SUPPLIERID FROM SUPPLIERS WHERE _NAME = @cName),-1)
	RETURN @nReturnID
...
--Added by Sojan E John KTS Infotech
----------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[usp_supportcontractasset_enumerate]

AS 
	DECLARE @cSQL varchar(1024) 
	--SET nocount ON 	
BEGIN
	SELECT _CONTRACT_NUMBER,_CONTRACT_VALUE ,_SUPPORT_EXPIRY ,_ALERTFLAG ,_NOOFDAYS
	,_ALERTBYEMAIL ,_NOTES 	,_ASSETID, _SUPPLIERID
...
--SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[usp_contractid_byAssetid]
	@itemID int,
	@retValue varchar(900) output
...
BEGIN
	SET @retValue = (SELECT _CONTRACT_NUMBER FROM ASSET_SUPPORTCONTRACT WHERE _ASSETID =@itemID)
...
CREATE PROCEDURE [dbo].[usp_supportcontract_assetid]
	@itemID int
...
AS 
	...
	--SET nocount ON 	
BEGIN
	SELECT _CONTRACT_NUMBER ,_CONTRACT_VALUE ,_SUPPLIERID ,_SUPPORT_EXPIRY ,_ALERTFLAG ,_NOOFDAYS
	,_ALERTBYEMAIL ,_NOTES, _ASSETID 
 
        FROM ASSET_SUPPORTCONTRACT WHERE _ASSETID =@itemID
...
--																		--
-- This stored procedure is part of the AuditWizard Product				--
-- It is used to check for any asset support alerts having been triggered		--
--																		--
--------------------------------------------------------------------------
--																		--
-- History																--
-- =======																--
--																		--
--	30-March-2010	Sojan E John		Support contracts for assets enhancement							--
--																		--
--------------------------------------------------------------------------

CREATE PROCEDURE [dbo].[usp_check_support_assetalerts]
AS
...
SET @dtToday = GETDATE();
	...
	SELECT     ASSET_SUPPORTCONTRACT._SUPPORT_EXPIRY, ASSET_SUPPORTCONTRACT._NOOFDAYS, ASSET_SUPPORTCONTRACT._ALERTBYEMAIL, 
                      ASSETS._NAME
FROM         ASSET_SUPPORTCONTRACT INNER JOIN
                      ASSETS ON ASSET_SUPPORTCONTRACT._ASSETID = ASSETS._ASSETID
WHERE     (_ALERTFLAG = 1) AND (ASSET_SUPPORTCONTRACT._NOOFDAYS <> - 1) AND 
                      (ASSET_SUPPORTCONTRACT._SUPPORT_EXPIRY - ASSET_SUPPORTCONTRACT._NOOFDAYS <= @dtToday)
