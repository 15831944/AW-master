...
*******************************************************************************/
int CSelfExtractor::Create(LPCSTR pszExtractorPath, LPCSTR pszFilename, funcPtr pFn /* = NULL */, void* userData /*=NULL*/)
{
	...
	shOp.AddFile(SH_SRC_FILE, pszExtractorPath);
	shOp.AddFile(SH_DEST_FILE, pszFilename);
	if(shOp.CopyFiles() != 0)
		return COPY_FAILED;

	// Open the archive
	CFile file;
	if(!file.Open(pszFilename, CFile::modeWrite))
...
*******************************************************************************/
BOOL CSelfExtractor::AddFile(LPCSTR pszFile)
{
	if(m_nFiles == MAX_FILES)
		return FALSE;

	if(m_InfoArray[m_nFiles].SetData(pszFile))
	{
		m_nFiles++;
		return TRUE;
	}
