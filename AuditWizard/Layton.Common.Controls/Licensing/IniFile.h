...
// FILE:	IniFile.h
// PURPOSE:	Declaration of INI File wrapper class for 32 bit MFC Applications
// AUTHOR:	JRF Thornley - Copyright (C) InControl Desktop Systems Ltd 1999,2000
// HISTORY:	JRFT - 25.03.1999 - created
//			JRFT - 28.09.2000 - now requires no explicit initialisation
//
...
//			in RAM and only flushed to the file when the object is destroyed, though
//			a flush can be explicitly requested when a value is written.
// USAGE:	1. Include this file in stdafx.h - must follow an inclusion of "arrays.h"
//			2. Add IniFile.cpp to your Project
//			3. Add a public member object to the application class "CIniFile m_IniFile"
//			5. To read values call SetSection() then either GetString(), GetInt(), GetBool()
//			6. To write values call SetSection() then either WriteString(), WriteInt(), WriteBool()
//			7. To list section keys call EnumKeys(), to enumerate sections call EnumSections()
//			8. For easy access define a macro in the application class header file
//					#define GetINI() (((CYourAppClass*)AfxGetApp())->m_IniFile)

#ifndef _INIFILE_DEF_
#define _INIFILE_DEF_
