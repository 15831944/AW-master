...
namespace Layton.Common.Controls
{
	...
	public class GlacialList : System.Windows.Forms.Control
	{
		...
		private Layton.Common.Controls.ManagedVScrollBar vPanelScrollBar;
		private Layton.Common.Controls.ManagedHScrollBar hPanelScrollBar;
		...
		public GlacialList()
		{
			...
			this.hPanelScrollBar = new Layton.Common.Controls.ManagedHScrollBar();
			this.vPanelScrollBar = new Layton.Common.Controls.ManagedVScrollBar();
		...
		/// <summary>
		/// Ensure that the item with index 'index' is visible
		/// </summary>
		/// <param name="index">Index of the item to ensure is visible</param>
		public void EnsureVisible(int index)
		{
			if (index < this.vPanelScrollBar.Value) // its out of viewable, move the surface
				this.vPanelScrollBar.Value = index;
			if (index > (this.vPanelScrollBar.Value + (this.VisibleRowsCount - 1)))
				this.vPanelScrollBar.Value = index - (this.VisibleRowsCount - 1);
		}


		/// <summary>
		/// Ensure that the specified item is visible in the view
		/// </summary>
		/// <param name="item">The Item to be made visible</param>
		public void EnsureVisible(GLItem item)
		{
			int index = Items.FindItemIndex(item);
			EnsureVisible(index);
		}
		...
		virtual public void DrawCellText( Graphics graphicsCell, Rectangle rectCell, string strCellText, ContentAlignment alignment, Color textColor, bool bSelected, bool bWordWrap  )
		{
			...
			MaxHeight = (int)measuredSize.Height + (CellPaddingSize*2);
													// this will only set if autosize is true
