...
#define FILE_ATTRIBUTE_NORMAL CFile::normal
  ...
  enum Attribute {
	...
	//@cmember
	/* Renames a file.*/
	BOOL RenameFile(const CString& OldFileName, const CString& NewFileName ,BOOL bOverwrite=FALSE);
	...
	CStringList* GetFileList(const CString& SearchString, const unsigned long eFileAttrib ,BOOL bNameOnly=FALSE);
