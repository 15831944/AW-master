...
bool CMarkup::Load(LPCTSTR szDoc, CString& szContents)
{
	CFile file;
	if ( ! file.Open( szDoc, CFile::modeRead ) )
	{
		return false;
	}
...
bool CMarkup::Save(LPCTSTR szDoc, CString szContents)
{
	CFile file;
	if (!file.Open(szDoc, CFile::modeCreate | CFile::modeWrite))
	{
		return false;
	}
	file.Write(szContents, szContents.GetLength());
	file.Close();
	return true;
}
