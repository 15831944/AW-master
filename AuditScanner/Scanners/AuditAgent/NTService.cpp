	...
	, m_pszPassword(0)
{
	...
	if (m_bWinNT)
	{
		...
		if ( ::LookupAccountName (0, sUserName, &security_identifier_buffer,
			&dwSizeSecurityIdBuffer, sDomainName, &dwSizeDomainName, &sidTypeSecurityId))
		{
...
void WINAPI CNTService :: ServiceCtrl(DWORD dwCtrlCode) 
{
	...
	CLogFile log;
	//log.Format("Service control message [%d] received\n" ,dwCtrlCode);
	...
	switch( dwCtrlCode ) 
	{
		...
		default:
			// invalid or custom control code - pass to the over-ridden custom command handler
			gpTheService->CustomCommand(dwCtrlCode);
			break;
	}
